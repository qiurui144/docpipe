...
static RE_PLATE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼][A-HJ-NP-Z][A-HJ-NP-Z0-9]{5}").unwrap()
});
...
pub fn is_valid_cn_id(s: &str) -> bool {
    ...
    if c.len() != 18 || !c[..17].iter().all(|x| x.is_ascii_digit()) {
        return false;
    }
...
pub fn detect_regex(text: &str) -> Vec<PiiEntity> {
    ...
    push_matches(text, &RE_BANK, PiiKind::BankCard, &mut out, |s| {
        !is_valid_cn_id(s) && luhn_ok(s)
    });
...
mod tests {
    ...
    fn cn_id_checksum_rejects_invalid() {
        ...
        let e: Vec<_> = detect_regex("证件 110105194912310021 末位错")
            .into_iter()
            .filter(|e| e.kind == PiiKind::IdCard)
            .collect();
